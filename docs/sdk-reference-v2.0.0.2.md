# BroSDK v2.0.0.2 接入参考文档

> **当前版本：v2.0.0.2　　最后更新：2026-06-29**   本文档是 BroSDK v2.0.0.2 的统一接入参考，覆盖动态库接口与本地 Web API。文档面向 SDK 接入方，只描述公开接口、行为语义、接入原则和排障口径，如字段或行为与头文件存在差异，请以随包发布的 `brosdk.h` 为最终依据。
...
`browser-open-success` 是浏览器可用的最终信号。收到该事件后，接入方可以认为浏览器进程已运行、SDK 运行态已就绪，后续可以执行 CDP 命令、打开页面或进行业务操作。

HTTP ACK 或动态库异步函数返回受理状态，只表示任务进入调度，不表示浏览器可用。

### 5.3 调试端口

SDK 会保证浏览器运行态可用。接入方只需要关注公开接口和明确声明支持的启动参数。

如接入方需要把浏览器调试端口开放给自己的工具，可以在 `browser/open` 的 `envs[].args` 中传入 Chromium 兼容的 `--remote-debugging-port`。

| 场景 | 行为 |
| --- | --- |
| 不传 `--remote-debugging-port` | 浏览器正常启动；`remoteDebuggingPort` 可能为 `0` |
| 传入 `--remote-debugging-port=9222` | SDK 保留该用户参数；最终事件和 `sdk_browser_info` 返回 `9222` |
| 传入 `--remote-debugging-port=0` | SDK 自动分配本机可用端口，并在最终事件和 `sdk_browser_info` 返回实际端口 |

`browser-open-success` 中的 `data.remoteDebuggingPort` 是本次环境对外可见的调试端口摘要。需要在打开后查询当前运行环境时，可调用 `sdk_browser_info` 或 `POST /sdk/v1/browser/info`。
...
| `args` | `browser/open` 本次启动参数 | Chromium 兼容命令行参数；代理旁路公开口径见 `--proxy-bypass-list` 与 `--bro-proxy-bypass-list` |
...
v2.0.0.2 的代理收口以 `docs/bridgeProxy.mmd` 中的流程图为准。核心原则是：浏览器流量先进入 SDK 本地桥，由本地桥统一承载代理例外、代理出口选择和代理链路诊断。
...
| 配置不走代理的目标 | 原生 Chromium 直连使用 `--proxy-bypass-list`；需要保留 SDK 桥内诊断时使用 `--bro-proxy-bypass-list` |
...
`args` 中可以携带 Chromium 参数。代理旁路分为两类：

```text
--proxy-bypass-list
--bro-proxy-bypass-list

```

`--proxy-bypass-list` 保持 Chromium 原生命令行语义，由浏览器执行 DIRECT 路由。命中后请求不会进入本地桥，因此不会产生桥内代理诊断或桥内阻断日志。该参数可以使用 Chromium 支持的旁路语法；SDK 不承诺把其中所有复杂语义转换为桥内策略。

`--bro-proxy-bypass-list` 是 BroSDK 桥内代理旁路参数。命中的请求仍先进入本地桥，桥完成基础诊断和策略处理后直连目标，不使用 `forward`、`proxy`、`bridgeProxy` 或系统代理出口。该参数仅承诺域名/主机规则，使用英文逗号 `,` 或英文分号 `;` 分隔。

SDK 会统一管理浏览器代理入口。接入方不应把其他 Chromium 代理启动参数作为 v2.0.0.2 的公开接入口径。
...
> 本文件对应 SDK v2.0.0.2；本地 Web API 路径当前仍使用 `/sdk/v1/...`。不要把路径版本号理解为 SDK 版本号。`getUserSig` 当前作为动态库 `sdk_get_user_sig` 暴露，不作为稳定本地 HTTP 入口。
...
| `urls[]` | 默认在浏览器 CDP 就绪且 Cookie / Storage 恢复完成后打开，避免页面先于登录态数据加载；仅当进程环境变量 `BROSDK_STARTUP_URLS_AS_ARGS=1` 时，SDK 才会把 URL 作为 Chromium 启动位置参数传入，此兼容模式下页面可能早于 CDP Cookie 注入加载 |
...
| `reloadAfterCookieSet` | 否 | 仅对 `Network.setCookie`、`Network.setCookies`、`Storage.setCookies` 生效。设为 `true` 时，命令成功后 SDK 会触发页面刷新：如果传入 `sessionId`，刷新该 session；否则刷新当前环境内 page targets。默认 `false`。 |

运行中通过 CDP 植入 Cookie 后，已经加载完成的页面不会自动重新发起带 Cookie 的请求。若希望页面立即按新 Cookie 判断登录态，可在 Cookie 写入命令中传入 `reloadAfterCookieSet: true`，或由接入方自行追加 `Page.reload` / 重新导航。
...
`sdk_env_get_cookies` 当前仅为历史头文件声明，未形成稳定实现和接入口径，不作为 v2.0.0.2 对外公开接口。
...
| 代理旁路参数 | `--proxy-bypass-list` 为 Chromium 原生直连规则；`--bro-proxy-bypass-list` 为 SDK 桥内旁路规则 |
